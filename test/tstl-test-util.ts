...
/**
 * This file is derived from TypescriptToLua/test/util.ts.
 *
 * TypeScriptToLua is licensed under the MIT license.
 */

import * as nativeAssert from "assert"
import * as fs from "fs"
import { stringify } from "javascript-stringify"
import { LauxLib, Lua, LUA_OK, LuaLib, LuaState } from "lua-wasm-bindings/dist/lua"
...
expect.extend({
  toHaveDiagnostics(diagnostics: ts.Diagnostic[], expected?: number[]): jest.CustomMatcherResult {
    ...
    return {
      pass: expected
        ? diagnostics.length === expected.length && diagnostics.every((diag, index) => diag.code === expected[index])
...
export abstract class TestBuilder {
  ...
  public getProgram(): ts.Program {
    ...
    // internal
    return (tstl as any).createVirtualProgram(
      { ...nonLuaExtraFiles, [this.mainFileName]: this.getTsCode() },
      this.options,
    )
  }
  ...
  public expectDiagnosticsToMatchSnapshot(expected?: number[], diagnosticsOnly = false): this {
    ...
    const diagnosticMessages = ts.formatDiagnostics(this.getLuaDiagnostics().map(tstl.prepareDiagnosticForFormatting), {
      getCurrentDirectory: () => "",
